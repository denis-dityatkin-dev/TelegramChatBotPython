...
class ChatGPT3TelegramBot:
    ...
    async def hello(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        """
        await update.message.reply_text("Привіт! Я - ваш AI помічник. Що б ви не запитали - я з радістю відповім!",
                                        disable_web_page_preview=True)                                   
    ...
    async def prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        while True:
            try:
                # Очікуємо, доки користувач надішле повідомлення
                user_input = await self.wait_for_user_input(update, context)

                # Відправляємо повідомлення користувача в OpenAI, щоб отримати відповідь
                response = self.openai.get_chat_response(chat_id=chat_id, query=user_input)

                # Відправляємо відповідь від OpenAI користувачу
                ...
                # Якщо користувач не надсилає повідомлення протягом періоду очікування,
                # завершити розмову і сказати прощання
                await context.bot.send_message(
                    chat_id=chat_id,
                    reply_to_message_id=message.message_id,
                    parse_mode=constants.ParseMode.MARKDOWN,
                    text="Дякуємо за спілкування! Бот завершує роботу."
                )
                break
            except Exception as e:
                # Обробка будь-яких інших помилок, які можуть виникнути під час розмови
                logging.error(str(e))
                await context.bot.send_message(
                    chat_id=chat_id,
                    reply_to_message_id=message.message_id,
                    parse_mode=constants.ParseMode.MARKDOWN,
                    text="Сталася помилка при обробці вашого повідомлення. Спробуйте ще раз."
                )
    ...
    async def is_valid_message(self, update: Update) -> bool:
        """
    ...
    def run(self):
        ...
        application.add_handler(CommandHandler('start', self.hello))
